...
class TdbfCtlAppSelectAction {
	var $CTLID='TdbfCtlAppSelectAction';
	var $TDBFPARAM;
	var $CI;
	var $thisInParent;
	var $TDBFModel;
	...
	var $ExtIncludeFile=array(
								'js'=>array('myjs/TdbfCtlAppAction.js'),
								'css'=>array('mycss/TdbfCtlAppAction.css')
							);

	public function init(&$pCtlObj,&$thisInParent,$pUserInfo){
		//$this->CI =& get_instance();
		$this->CI =$pCtlObj;
		$pStrJSON=file_get_contents($this->CI->config->item('data_model_path').'AppAction.tdbf');
	...
	public function display($pSection='',&$pMainPageLoadMode){
		switch ($pSection) {
			case 'view':$this->CI->TdbfSystem->checkAllowedPrivilegeAction('lihat_app_action'); break;
			case 'exportExcel':$this->CI->TdbfSystem->checkAllowedPrivilegeAction('lihat_app_action'); break;
			case '':$this->CI->TdbfSystem->checkAllowedPrivilegeAction('lihat_app_action'); break;
		}
		$pStrHTML='';
	...
	public function getDataRecordPreSelectionStatus($pDtRec,$pSelectParentFormId,$pSelectSessionCode){
		...
		switch($pSelectParentFormId.'/'.$pExtractSelectSessionCode[0]){
			case 'TdbfCtlAppPrivilege/sessAppSelectPrivilegeAction':
				$pStrSql='select count(*) from tbl_privilege_action where `privilege_id`="'.$pExtractSelectSessionCode[1].'" and `app_action_id`="'.$pDtRec['id'].'"';

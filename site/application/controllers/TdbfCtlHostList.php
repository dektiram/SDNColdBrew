...
class TdbfCtlHostList {
	var $CTLID='TdbfCtlHostList';
	var $TDBFPARAM;
	var $CI;
	var $thisInParent;
	var $TDBFModel;
	...
	var $ExtIncludeFile=array(
								'js'=>array('myjs/TdbfFunctionCollection.js','myjs/TdbfCtlHostList.js'),
								'css'=>array('mycss/TdbfCtlHostList.css')
							);

	public function init(&$pCtlObj,&$thisInParent,$pUserInfo){
		//$this->CI =& get_instance();
		$this->CI =$pCtlObj;
		$pStrJSON=file_get_contents($this->CI->config->item('data_model_path').'HostList.tdbf');
	...
	public function onRecView($pDtRec,$pFieldName){
		...
		switch ($pFieldName) {
			case 'route':
				$pStrColView = '<pre>'.$pDtRec[$pFieldName].'</pre>';
				break;
			default:
	...
	public function onExtendedColView($pDtRec,$pColName){
		...
		switch($pColName){
			case 'ipv4':
				$strSql = 'select * from `t_host_ipv4` where `hw_addr`="'.$pDtRec['hw_addr'].'" order by `index`';
				...
				break;
			case 'ipv6':
				$strSql = 'select * from `t_host_ipv6` where `hw_addr`="'.$pDtRec['hw_addr'].'" order by `index`';
				$x1 = $this->TDBFModel->tdbfGetnxnResult($strSql, TRUE);
				foreach ($x1 as $key => $value) {
					$pStrColView .= $value['addr'].'/'.$value['prefix'].'<br />';
				}

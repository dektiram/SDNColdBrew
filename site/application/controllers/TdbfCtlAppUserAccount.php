...
class TdbfCtlAppUserAccount {
	var $CTLID='TdbfCtlAppUserAccount';
	var $TDBFPARAM;
	var $CI;
	var $thisInParent;
	var $TDBFModel;
	...
	var $ExtIncludeFile=array(
								'js'=>array('myjs/TdbfFunctionCollection.js','myjs/TdbfCtlAppUserAccount.js'),
								'css'=>array('mycss/TdbfCtlAppUserAccount.css')
							);

	public function init(&$pCtlObj,&$thisInParent,$pUserInfo){
		//$this->CI =& get_instance();
		$this->CI =$pCtlObj;
		$pStrJSON=file_get_contents($this->CI->config->item('data_model_path').'AppUserAccount.tdbf');
	...
	public function onExtendedColView($pDtRec,$pColName){
		...
		switch($pColName){
			case 'privilege':
				$pFormUrl=base_url().'Administrator/appUserPrivilege';
				$s1=$this->TDBFPARAM['CTLID'].'.myOpenSelectForm('.chr(39).$pFormUrl.chr(39).','.chr(39).'sessAppUserPrivilege/'.$pDtRec['id'].chr(39).');';
				$pStrColView='<span class="glyphicon glyphicon-check myPointerCursor" onclick="'.$s1.'"></span>';
				break;
			case 'outletId':
				$pFormUrl=base_url().'Administrator/selectOutlet';
				$s1=$this->TDBFPARAM['CTLID'].'.myOpenSelectForm('.chr(39).$pFormUrl.chr(39).','.chr(39).'sessSelectAppUserOutletId/'.$pDtRec['id'].chr(39).');';
				$pStrColView='<span class="glyphicon glyphicon-check myPointerCursor" onclick="'.$s1.'"></span>';
				break;
		}
		return $pStrColView;

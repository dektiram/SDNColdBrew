...
class TdbfCtlAppSettings {
	var $CTLID='TdbfCtlAppSettings';
	var $TDBFPARAM;
	var $CI;
	var $thisInParent;
	var $TDBFModel;
	...
	var $ExtIncludeFile=array(
								'js'=>array('myjs/TdbfFunctionCollection.js','myjs/TdbfCtlAppSettings.js'),
								'css'=>array('mycss/TdbfCtlAppSettings.css')
							);

	public function init(&$pCtlObj,&$thisInParent,$pUserInfo){
		//$this->CI =& get_instance();
		$this->CI =$pCtlObj;
		$pStrJSON=file_get_contents($this->CI->config->item('data_model_path').'AppSettings.tdbf');
